...
async function spawnManagedServiceUpdateHandoff(
  ...
  const selectedEnv =
    params.devBranch === undefined
      ? readyEnv
      : { ...readyEnv, [UPDATE_DEV_BRANCH_ENV]: params.devBranch };
  const env = params.devTarget
    ? applyDevUpdateTargetEnv(selectedEnv, params.devTarget)
    : selectedEnv;
